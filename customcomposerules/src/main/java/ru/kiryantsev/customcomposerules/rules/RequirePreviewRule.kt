package ru.kiryantsev.customcomposerules.rules
...
class RequirePreviewRule(config: Config = Config.empty) : Rule(config) {
    ...
    override val issue: Issue = Issue(
        id = "require-preview-rule",
        severity = Severity.Style,
        description = "Each Composable function must have a corresponding Preview function, unless it is marked as ReadOnlyComposable.",
        debt = Debt.TEN_MINS
    )
            ...
            if (!hasPreviewFun) {
                report(
                    CodeSmell(
                        ...
                        "Composable function '${composableFunRequirePreview.name}' does not have a corresponding Preview function."
                    )
                )
            }
        }
